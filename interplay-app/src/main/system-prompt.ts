...
**p5.js側 WebSocket OSC受信の実装（osc-jsは使わない）:**
osc-js CDN はブラウザ環境で読み込みに失敗することが多いため、WebSocket で直接接続し OSC バイナリを自前パースする。以下のヘルパー関数をスケッチに含めること：

\`\`\`javascript
// --- OSC WebSocket helper (include in every OSC sketch) ---
let oscWs = null;
let oscData = {}; // key: address, value: latest float value
...
function parseOSC(buf) {
  ...
  let i = 0;
  // read address string (null-terminated, 4-byte aligned)
  let addrEnd = i;
  while (addrEnd < view.byteLength && view.getUint8(addrEnd) !== 0) addrEnd++;
  let address = String.fromCharCode.apply(null, new Uint8Array(buf, i, addrEnd - i));
  i = addrEnd;
  i += 4 - (i % 4); // align to 4 bytes
  // read type tag
  if (i >= view.byteLength || view.getUint8(i) !== 44) return null; // expect ','
  i++;
  let type = String.fromCharCode(view.getUint8(i));
  i++;
  i += 4 - (i % 4); // align
  // read value
  let value = 0;
...
function sendOSC(address, value) {
  ...
  // build minimal OSC message: address + ,f + float
  let addrBytes = new TextEncoder().encode(address);
  let addrLen = addrBytes.length + 1; // +1 for null
  addrLen += (4 - (addrLen % 4)) % 4; // pad to 4
  let tagLen = 4; // ",f\\0" + padding
  let buf = new ArrayBuffer(addrLen + tagLen + 4);
...
- 外部ライブラリ（p5.sound等）は使用可能。ただし **osc-js は使わない**（CDN読み込み失敗が多いため）。OSC通信には上記の WebSocket 直接接続 + 自前パースのヘルパー関数を使うこと
